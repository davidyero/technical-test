...
export const SuperActionButtons = ({productId}: SuperActionButtonsProps) => {
  ...
  const copyProduct = (event: React.MouseEvent<HTMLImageElement>, id: string) => {
  ...
  return (
    <div className="action-buttons-container">
      <img onClick={(event) => copyProduct(event, productId)} className="card-product--icon" src={CopyIcon}/>
      {isAdmin && <img onClick={editProduct} className="card-product--icon" src={EditIcon}/>}
      {isAdmin && <img onClick={removeProduct} className="card-product--icon" src={TrashIcon}/>}
    </div>
  )
}
